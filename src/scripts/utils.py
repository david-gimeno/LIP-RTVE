...
def read_pkl(landmarks_path):
    """read_pkl.
        :param landmarks_path: str, path to the PKL file which stores the corresponding landmarks
    """
    with open(landmarks_path, "rb") as pkl_file:
        landmarks = landmarks_interpolate(pickle.load(pkl_file))
...
def get_source_rtve2018_path(source_dir, sourceID):
    """get_source_rtve2018_path.
    ...
    """
    source_train = [tv_programme.strip() for tv_programme in open("./src/scripts/resources/RTVE2018-20H-train-programmes.txt").readlines()]
    source_dev = [tv_programme.strip() for tv_programme in open("./src/scripts/resources/RTVE2018-20H-dev-programmes.txt").readlines()]

    if sourceID in source_train:
        return os.path.join(source_dir, "train", "20H", sourceID + ".mp4")
    else:
        return os.path.join(source_dir, "dev", "mp4", "20H", sourceID + ".mp4")
